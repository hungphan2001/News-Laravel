...
<div class="card card-primary">
    ...
    <div class="card-body">
     <form action="{{ route('admin.language.update',$language->id) }}" method="post">
        ...
        <div class="form-group">
            <label for="">{{ __('Is is default') }}?</label>
        ...
        <div class="form-group">
            ...
            <select name="status" id="" class="form-control select2" value={{ $language->status }}>
                <option {{ $language->default===1 ? 'selected': '' }} value="1">{{ __('Active') }}</option>
                <option {{ $language->default===0 ? 'selected': '' }} value="0">{{ __('Inactive') }}</option>
            </select>
            @error('status')
            <p class="text-danger">{{ $message }}</p>
            @enderror
